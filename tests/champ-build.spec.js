...
for (const site of SITES) {
  for (const champ of champions) {
    for (const name of champ.names) {
      allTests.push({ site, champ, name });
    }
  }
}
const shuffledTests = shuffle(allTests);
...
test.describe("Webpage Load Test", () => {
  ...
  test.describe("Loads Correct Champion Build Page", () => {
    for (const { site, champ, name } of shuffledTests) {
      test(`${site.name} - Omnibox: "${name}"`, async ({ page, context }) => {
